...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchName = async () => {
      // FetchName function. Gets customers name.
...
const DisplayGreeting = (props: { name; dictionary }) => {
  if (props.name !== null) {
    // Checks if a user is logged in and name has been fetched.
    return (
      <h1>
    ...
    // If no name registred, just print welcome message without name.
...
const DisplayCustomers = (props: { filter; name; dictionary; selectedCutomersArray; updateFunction }) => {
  ...
  if (props.name !== null) {
    ...
    // If props.name is null, show loadingsymbol from imported LoadingComponent.
